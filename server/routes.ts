    ...
    try {
      ...
      res.json({ 
        ...
        conversionRate:dbUser.conversionRate,
         ...
         withdrawalMin:dbUser.withdrawalMin,
         withdrawalMax:dbUser.withdrawalMax
      });
    } catch (error) {
...
app.get("/api/user/update-level", async (req: Request, res: Response) => {
  try {
    const userEmail = await getUser(req);
    ...
    return res.json({
      success: true,
      artworksCount: listedCount,
      previousLevel: currentLevel,
      newLevel: calculatedLevel,
      updated: currentLevel !== calculatedLevel
    });
